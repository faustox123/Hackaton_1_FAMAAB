...
import Graph.Edge;
import Graph.Graph;
import Graph.LinkedPositionalList;
import Graph.Map;
...
public class AdjacencyMapGraph<V,E> implements Graph<V,E> {
  ...
  public Edge<E> insertEdge(Vertex<V> u, Vertex<V> v,E element)
    ...
    if (getEdge(u,v) == null) {
      ...
      origin.getOutgoing().put(v, e);       //Associates the given value with the given key. If an entry with
      dest.getIncoming().put(u, e);            // the key was already in the map, this replaced the previous value
      return e;                                        // with the new one and returns the old value. Otherwise, a new
    } else                                             // entry is added and null is returned.*/                                          
